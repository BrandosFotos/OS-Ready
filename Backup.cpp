...
// Prototype functions
void clearscreen();
void menu(std::string& username, std::string& playbook, std::string& platform, bool remote);
...
int main() {
    ...
    menu(username, playbook, platform, remote);
    return 0;
}
...
void menu(std::string& username, std::string& playbook, std::string& platform,bool remote) {
    ...
    do {
        ...
        case 4:
            ...
            std::cout << "-----------------------------------\n";
            playbook = platform + "/Install_Mac_Apps.yaml";

            system(("ansible-playbook " + playbook).c_str());
...
void exportToInventory(const std::string& title, const std::string& keyPath, const std::string& username, const std::string& hostname, const std::string& password) {
    std::string inventoryPath = "inventory.ini";
    std::ofstream inventoryFile(inventoryPath, std::ios::app); // Open in append mode
    bool inventoryFileExists = std::ifstream(inventoryPath).good();

    if (!inventoryFileExists) {
        std::cerr << "inventory.ini not found. Creating a new one...\n";
        std::ofstream newInventoryFile(inventoryPath);
        newInventoryFile << "[mesh_devices]\n";
